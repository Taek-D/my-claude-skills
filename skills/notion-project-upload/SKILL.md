...
> ⚙️ **First-time setup required**: Before using this skill, configure your Notion database.
> See [README.md](../../README.md#notion-database-setup) for step-by-step instructions.

- **Data Source ID**: Set via Claude user memory or provide at upload time
  ```
  Example: "노션에 업로드해줘. DB ID는 YOUR_DATABASE_ID야"
  ```
- **Recommended**: Add your DB ID to Claude's user memory so it's auto-applied every session
